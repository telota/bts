...
public class BTSTextServiceImpl extends AbstractCorpusObjectServiceImpl<BTSText, String> implements BTSTextService, BTSObjectSearchService
{
	...
	public BTSText findReturnOnlyMetadata(String key) {
		BTSText text = textDao.findReturnOnlyMetadata(key, main_corpus_key);
		if (text != null)
			return text;
	...
	public BTSSentenceItem copySentenceItem(BTSSentenceItem item) {
        BTSSentenceItem newItem = EcoreUtil.copy(item);
        createIds(newItem);
        return newItem;
	}
	...
	private void createIds(BTSIdentifiableItem item) {
        ...
        if (item instanceof BTSSenctence) {
            for (BTSSentenceItem seItem : ((BTSSenctence)item).getSentenceItems())
                createIds(seItem);
	...
	private WordFormOccurrence processToken(BTSSentenceItem sentenceItem, BTSSenctence sentence, int sentenceIndex, BTSText text, BTSQueryRequest query,
		...
		if (WordFormOccurrenceGroup.GROUP_LEMMA.equals(query.getGroupBy())) { // lemma
			if (word.getLKey() != null && word.getLKey().equals(query.getSearchString()))
				return addToGroupMap(word.getLKey(), word, sentence, sentenceIndex, text, occurrenceGroupsMap, WordFormOccurrenceGroup.GROUP_TRANSLITERATION);

            return null;
        }
        
        if (WordFormOccurrenceGroup.GROUP_FLEXION.equals(query.getGroupBy())) { // flexion
			if (word.getFlexCode() != null && word.getFlexCode().equals(query.getSearchString()))
				return addToGroupMap(word.getFlexCode(), word, sentence, sentenceIndex,  text, occurrenceGroupsMap, WordFormOccurrenceGroup.GROUP_TRANSLITERATION);
            
            return null;
		}
        
        if (WordFormOccurrenceGroup.GROUP_SPELLING.equals(query.getGroupBy())) {
			// spelling
            ...
            if (query.isFuzzy() && mdc.contains(query.getSearchString()))
                return addToGroupMap(word.getGraficsAsMdC(), word, sentence, sentenceIndex,  text, occurrenceGroupsMap, WordFormOccurrenceGroup.GROUP_TRANSLITERATION);

            if (mdc.equals(query.getSearchString()))
                return addToGroupMap(word.getGraficsAsMdC(), word, sentence, sentenceIndex,  text, occurrenceGroupsMap, WordFormOccurrenceGroup.GROUP_TRANSLITERATION);
            
            return null;
		}
        
        if (WordFormOccurrenceGroup.GROUP_TRANSLATION.equals(query.getGroupBy())) {
			// translation
            ...
            if (word.getTranslation().getTranslation(query.getLang()).equals(query.getSearchString()))
                return addToGroupMap(word.getTranslation().getTranslation(query.getLang()), word, sentence,  sentenceIndex, text, occurrenceGroupsMap, WordFormOccurrenceGroup.GROUP_TRANSLITERATION);

            return null;
		}
        
        if (WordFormOccurrenceGroup.GROUP_TRANSLITERATION.equals(query.getGroupBy())) { // transliteration
			if (word.getWChar() == null)
                return null;

            if (query.isFuzzy() && word.getWChar().contains(query.getSearchString()))
                return addToGroupMap(word.getWChar(), word, sentence,  sentenceIndex, text, occurrenceGroupsMap, WordFormOccurrenceGroup.GROUP_TRANSLITERATION);
        ...
        if (word.getWChar() != null && word.getWChar().equals(query.getSearchString()))
            return addToGroupMap(null, word, sentence,  sentenceIndex, text, occurrenceGroupsMap, WordFormOccurrenceGroup.GROUP_TRANSLITERATION);
	...
	private void processCoText(BTSText text, int sentenceIndex, int cotextindex, boolean before, WordFormOccurrence occurrence) {
		...
		while(sentences.size() < Math.abs(cotextindex)) {
			...
			if (i < 0 || i >= text.getTextContent().getTextItems().size())
                break;
	...
	public String findAsJsonString(String key, IProgressMonitor monitor) {
		String text = textDao.findAsJsonString(key, main_corpus_key);
	...
	public List<String> queryAsJsonString(BTSQueryRequest query, String objectState, IProgressMonitor monitor) {
		...
		String[] indexArray = buildIndexArray();
		objects.addAll(textDao.queryAsJsonString(query, indexArray, indexArray, objectState, false));
		return objects;
	}
