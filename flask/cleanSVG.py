...
class CleanSVG:
    def __init__(self, svgfile=None, svgstring=None, verbose=False):
        ...
        if svgfile and svgstring:
            raise ValueError('Can only process either svgfile or svgstring')
    ...
    def parseString(self, svgstring):
        self.root = etree.fromstring(svgstring.encode())
    ...
    def toString(self, pretty_print=False):
        ...
        if self.removeWhitespace:
            return re.sub(r'\n\s*' , '', etree.tostring(self.root).decode())
        else:
