...
public class ATextNavigatorPart extends NavigatorPart implements ScatteredCachingPart, SearchViewer, StructuredViewerProvider {

    protected TreeNodeWrapper orphanNode;
    @Inject
    private EventBroker eventBroker;
    @Inject
    private ATextNavigatorController atextNavigatorController;
    ...
    private void prepareTreeViewer(final TreeViewer treeViewer,
                ...
                if (selection.getFirstElement() != null
                    ...
                    if (tn.getObject() != null) {
                        ...
                        if (o instanceof BTSCorpusObject) {
                            atextNavigatorController.checkAndFullyLoad((BTSCorpusObject) o, true);
        ...
        if (sorter == null) {
            sorter = ContextInjectionFactory.make(BTSATextBySortKeyNameViewerSorter.class, context);
        }
        ...
        try {
            IRunnableWithProgress op = new IRunnableWithProgress() {
                ...
                public void run(IProgressMonitor monitor)
                    ...
                    List<BTSAbstractText> obs;
                    obs = atextNavigatorController
                            .getOrphanEntries(map,
                                    treeViewer.getFilters(), monitor);
                    storeIntoMap(obs, parentControl, true);
                    final List<TreeNodeWrapper> nodes = atextNavigatorController.loadNodes(obs, monitor, true);
        ...
        try {
            IRunnableWithProgress op = new IRunnableWithProgress() {
                ...
                public void run(IProgressMonitor monitor)
                    ...
                    storeIntoMap(obs, parentControl, true);
                    List<TreeNodeWrapper> nodes = atextNavigatorController.loadNodes(obs, monitor, true);
        ...
        if (localSelection.getFirstElement() != null
            ...
            if (localTreeNode.getObject() != null) {
                ...
                if (localCorpusObject instanceof BTSCorpusObject) {
                    atextNavigatorController.checkAndFullyLoad((BTSCorpusObject) localCorpusObject, true);
            ...
            IRunnableWithProgress op = new IRunnableWithProgress() {
                ...
                public void run(IProgressMonitor monitor)
                    ...
                    for (final TreeNodeWrapper parent : parents) {
                        final List<BTSAbstractText> children = atextNavigatorController
                                .findChildren(
                                        (BTSAbstractText) parent.getObject(),
                                        queryResultMap,
                                        mainTreeViewer,
                                        parent,
    ...
    protected void storeIntoMap(final List<BTSAbstractText> children,
    ...
    @Inject
    @Optional
    void eventReceivedNew(@EventTopic("model_atext_new_root/*") BTSObject object) {
        if ((object instanceof BTSAbstractText)) {
    ...
    @Inject
    @Optional
    void eventReceivedAdd(@EventTopic("model_atext_add/*") BTSObject object) {
        if ((object instanceof BTSAbstractText)
                && selection != null
                && ((TreeNodeWrapper) selection.getFirstElement()).getObject() instanceof BTSAbstractText) {
            final TreeNodeWrapper tn = atextNavigatorController.addRelation((BTSAbstractText) object,
    ...
    void eventReceivedUpdates(@EventTopic("model_update/async") Object object) {
        ...
        if (object instanceof BTSAbstractText && selection != null
                && selection.getFirstElement() instanceof BTSAbstractText) {
            // corpusNavigatorController.addRelation((BTSCorpusObject) object,
            // "partOf",
            // (BTSCorpusObject) selection.getFirstElement(), input);

            refreshTreeViewer((BTSAbstractText) object);
        } else if (object instanceof BTSAbstractText) {
            refreshTreeViewer((BTSAbstractText) object);
            ...
            if (((BTSModelUpdateNotification) object).getObject() instanceof BTSComment) {
                ...
                // comment, do nothing
            } else if (atextNavigatorController.handleModelUpdate(
                    (BTSModelUpdateNotification) object, queryResultMap,
                    viewHolderMap)) {
                refreshTreeViewer((BTSAbstractText) ((BTSModelUpdateNotification) object)
                        .getObject());
            }
...
//					mainTreeViewer.addSelectionChangedListener(selectionListener);
    ...
    public void reloadViewerNodes(final StructuredViewer viewer) {
        ...
        List<BTSAbstractText> objs = null;
        if (data instanceof List<?>) {
            objs = (List<BTSAbstractText>) data;
        }
        if (objs == null) return;
        // filter nodes
        ...
        if (objs != null && objs.size() > 0) {
            List<TreeNodeWrapper> nodes = atextNavigatorController.loadNodes(objs, null, true);
            rootNode.getChildren().addAll(nodes);
        } else {
            TreeNodeWrapper emptyNode = BtsviewmodelFactory.eINSTANCE.createTreeNodeWrapper();
    ...
    private boolean isFiltered(BTSAbstractText e, StructuredViewer viewer) {
    ...
    public String[] getTypesFilterTerms() {
        return new String[]{BTSConstants.ABSTRACT_TEXT};
    }
