...
import static org.lightcouch.CouchDbUtil.getElementAsInt;
import static org.lightcouch.CouchDbUtil.getElementAsLong;
...
/**
 * <p>
 * This class allows construction and sending of View query requests. The API
 * supports view queries for various data type results, and for pagination.
 * 
 * <h3>Usage Example:</h3>
 * 
 * <pre>
 * {
 * 	&#064;code
 * 	List&lt;Foo&gt; list = dbClient.view(&quot;example/foo&quot;).includeDocs(true).startKey(&quot;start-key&quot;).endKey(&quot;end-key&quot;).limit(10)
 * 			.query(Foo.class);
 * 
 * 	int count = dbClient.view(&quot;example/by_tag&quot;).key(&quot;couchdb&quot;).queryForInt(); // query for scalar values
 * 
 * 	// query for view entries
 * 	View view = dbClient.view(&quot;example/by_date&quot;).key(2011, 10, 15) // complex key example
...
public class View
{
	...
	private <V> V queryValue(Class<V> classOfV)
	{
		...
		try
		{
			...
			{ // expect exactly 1 row
				throw new NoDocumentException("Expecting exactly a single result of this view query, but was: "
						+ array.size());
			}
	...
	/**
	 * Queries a view for pagination, returns a next or a previous page, this
	 * method figures out which page to return based on the given param that is
	 * generated by an earlier call to this method, quering the first page is
	 * done by passing a {@code null} param.
	 * 
	 * @param <T>
	 *            Object type T
	 * @param rowsPerPage
	 ...
	 * @param param
	 *            The request parameter to use to query a page, or {@code null}
	...
	public <T> Page<T> queryPage(int rowsPerPage, String param, Class<T> classOfT)
	{
		...
		try
		{
			...
			JsonObject json = new JsonParser().parse(new String(Base64.decodeBase64(param.getBytes())))
					.getAsJsonObject();
		...
		{ // previous
			return queryPreviousPage(rowsPerPage, currentStartKey, currentStartKeyDocId, startKey, startKeyDocId,
					classOfT);
		} else
		{ // next
			...
			String startKey, String startKeyDocId, Class<T> classOfT)
	{
		...
		for (int i = 0; i < resultRows; i++)
		{
			...
			if (i == resultRows - 1)
			{ // last element (i.e rowsPerPage + 1)
				if (resultRows > rowsPerPage)
				{ // if not last page
			...
			String startKey, String startKeyDocId, Class<T> classOfT)
	{
		...
		// set keys for the previous page
		if (offset != (totalRows - rowsPerPage - 1))
		{ // if not first page
			page.setHasPrevious(true);
			jsonPrev.addProperty(START_KEY, currentStartKey);
			jsonPrev.addProperty(START_KEY_DOC_ID, currentStartKeyDocId);
	...
	/**
	 * @param key
	 *            The key value, accepts a single value or multiple values for
	 *            complex keys.
	 */
	...
	/**
	 * @param startKey
	 *            The start key value, accepts a single value or multiple values
	 *            for complex keys.
	 */
	...
	/**
	 * @param endKey
	 *            The end key value, accepts a single value or multiple values
	 *            for complex keys.
	 */
	...
	/**
	 * @param stale
	 *            Accept values: ok | update_after (update_after as of CouchDB
	 *            1.1.0)
	 */
	...
	/**
	 * @param skip
	 *            Skips <i>n</i> number of documents.
	 */
	...
	/**
	 * @param group
	 *            Specifies whether the reduce function reduces the result to a
	 *            set of keys, or to a single result. Defaults to false (single
	 *            result).
	 */
	...
	public View tempView(String id)
	{
		...
		for (String mapRed : dirList)
		{
			...
			if (MAP_JS.equals(mapRed)) tempView.setMap(def);
			else if (REDUCE_JS.equals(mapRed)) tempView.setReduce(def);
		}
		return this;
	}
