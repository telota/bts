...
/**
 * Provides methods to create and save CouchDB design documents. <h3>Usage
 * Example:</h3>
 * 
 * <pre>
...
public class CouchDbDesign
{
	...
	private static final String DESIGN_DOCS_DIR = "designdocs";
	 ...
	 * <p>
	 * This method will first try to find a document in the database with the
	 * same id as the given document, if it is not found then the given document
	 * will be saved to the database.
	 * <p>
	 * If the document was found in the database, it will be compared with the
	 * given document using {@code equals()}. If both documents are not equal,
	 * then the given document will be saved to the database and updates the
	 * existing document.
	 * 
	 * @param document
	 *            The design document to synchronize
	 * @return {@link Response} as a result of a document save or update, or
	 *         returns {@code null} if no action was taken and the document in
	 *         the database is up-to-date with the given document.
	 */
	...
	public DesignDocument getFromDesk(String id)
	{
		...
		if (elements.contains(VALIDATE_DOC))
		{ // validate_doc_update
