...
import e4.handler.E4ActionHandler;
...
//		new IdMapEntry(IWorkbenchCommandConstants.EDIT_PASTE, ST.PASTE),
    ...
    protected final Map<String, E4ActionHandler> allHandlers = Maps.newHashMap();
...
//		});
        ...
        if (workbench != null) {
            ...
            final EHandlerService handlerService = StaticAccessController.getService();
            final EContextService contextService = StaticAccessController.getContextService();
            contextService.activateContext(EMBEDDED_TEXT_EDITOR_SCOPE);
            shell.addDisposeListener(new DisposeListener() {
                public void widgetDisposed(DisposeEvent e) {
                    ...
                    contextService.deactivateContext(EMBEDDED_TEXT_EDITOR_SCOPE);
                }
            });
...
//		action.setHelpContextId(IAbstractTextEditorHelpContextIds.QUICK_ASSIST_ACTION);
    ...
    protected void setAction(String actionID, final IAction action) {
        ...
        if (action == null) {
            ...
            allHandlers.remove(actionID);
        } else {
            E4ActionHandler handler = new E4ActionHandler(action);
            if (action.getId() == null) {
                action.setId(actionID); // make sure the action ID has been set
            }
            allActions.put(actionID, action);
            allHandlers.put(handler.getActionDefinitionId(), handler);
        }
    }
